...
def _get_student_remedial_messages_from_sql(*, db: Session, student_id: int, limit: int) -> list[schemas.RemedialMessageOut]:
    fetch_limit = min(max(int(limit) * 4, int(limit)), 500)
    rows = (
        ...
        .filter(models.RemedialMessage.student_id == int(student_id))
...
def _run_nonblocking_recovery_followups(
    db: Session,
    *,
    student_id: int,
    class_row: models.MakeUpClass,
    ...
    try:
        ...
        logger.warning(
            "Non-blocking remedial recovery follow-up failure for class_id=%s student_id=%s: %s",
            class_row.id,
            student_id,
            exc,
        )
...
def _remedial_target_student_ids(
    db: Session,
    *,
    class_row: models.MakeUpClass,
    sections: list[str] | None = None,
) -> set[int]:
    target_sections = sections if sections is not None else _parse_sections_json(class_row.sections_json)
    student_ids: set[int] = set()
    for student in _students_matching_sections(db, target_sections):
        student_id = int(getattr(student, "id", 0) or 0)
        if student_id <= 0:
            continue
        if _student_has_remedial_access(db, student_id=student_id, class_row=class_row):
            student_ids.add(student_id)

    targeted_rows = (
        db.query(models.RemedialMessage.student_id)
        .filter(models.RemedialMessage.makeup_class_id == int(class_row.id))
        .all()
    )
    for row in targeted_rows:
        student_id = int(row[0] or 0)
        if student_id > 0:
            student_ids.add(student_id)
    return student_ids


def _remedial_event_scopes(
    db: Session,
    *,
    faculty_id: int | None = None,
    class_row: models.MakeUpClass | None = None,
    student_ids: set[int] | None = None,
    sections: list[str] | None = None,
) -> set[str]:
    scopes = {"role:admin"}
    resolved_faculty_id = int(faculty_id or getattr(class_row, "faculty_id", 0) or 0)
    if resolved_faculty_id > 0:
        scopes.add(f"faculty:{resolved_faculty_id}")

    resolved_student_ids: set[int] = {
        int(student_id)
        for student_id in (student_ids or set())
        if int(student_id) > 0
    }
    if class_row is not None:
        resolved_student_ids.update(
            _remedial_target_student_ids(
                db,
                class_row=class_row,
                sections=sections,
            )
    ...
    scopes.update(f"student:{student_id}" for student_id in sorted(resolved_student_ids))
    return scopes
...
def create_makeup_class(
    ...
    if used_manual_course_entry:
        ...
        _safe_mirror_event(
            ...
            scopes=_remedial_event_scopes(db, faculty_id=int(course.faculty_id or 0)),
        )
...
def regenerate_remedial_code(
    ...
    _safe_mirror_event(
        ...
        scopes=_remedial_event_scopes(db, class_row=class_row),
    )
...
def send_remedial_code_to_sections(
    ...
    _safe_mirror_event(
        ...
        scopes=_remedial_event_scopes(
            db,
            faculty_id=int(class_row.faculty_id or 0),
            student_ids=recipient_student_ids,
        ),
    )
    return schemas.RemedialSendMessageOut(
        class_id=class_row.id,
...
def cancel_makeup_class(
    ...
    linked_student_ids = {
        int(student_id or 0)
        for _message_id, student_id in linked_messages
    ...
    try:
        ...
        if mongo_db is not None:
            mongo_db["remedial_messages"].delete_many(
                {
                    "$or": [
                        {"class_id": class_row.id},
                        {"makeup_class_id": class_row.id},
...
def get_student_remedial_messages(
    ...
    if _mongo_read_preferred():
        try:
            ...
            logger.warning("Mongo remedial message read fallback to SQL after connection failure: %s", exc)
            return _get_student_remedial_messages_from_sql(db=db, student_id=student_id, limit=limit)
        if mongo_db is not None:
            try:
            ...
            sql_rows = _get_student_remedial_messages_from_sql(db=db, student_id=student_id, limit=limit)
            if sql_rows:
                logger.warning(
                    "Mongo remedial message read returned no rows for student_id=%s; using SQL fallback.",
                    student_id,
                )
                return sql_rows
            return []
        ...
        if _mongo_read_required():
            logger.warning(
                "Mongo remedial message read unavailable in strict mode for student_id=%s; using SQL fallback.",
                student_id,
            )
...
def mark_remedial_attendance(
    ...
    try:
        db.flush()
        _run_nonblocking_recovery_followups(
            db,
